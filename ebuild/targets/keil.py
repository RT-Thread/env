...
def ARMCC_Version():
    from proj_config import CONFIG as toolchain_config
    import subprocess
    import re

    config = getattr(toolchain_config, "TOOLCHAIN_CONFIG", None)
    if isinstance(config, dict):
        path = config.get("EXEC_PATH", "")
        platform = config.get("PLATFORM", getattr(toolchain_config, "PLATFORM", ""))
    else:
        path = getattr(toolchain_config, "EXEC_PATH", "")
        platform = getattr(toolchain_config, "PLATFORM", "")
...
class KeilProjectGenerator:
    """Keil project generator wrapper."""

    def __init__(self, env, project_name: str) -> None:
        self.env = env
        self.project_name = project_name

    def generate(self, target_name: str, groups: list) -> None:
        if os.path.isfile("template.uvprojx") and target_name not in ["mdk4"]:
            MDK5Project(self.env, self.project_name + ".uvprojx", groups)
            print("Keil5 project is generating...")
        elif os.path.isfile("template.uvproj") and target_name not in ["mdk5"]:
            MDK4Project(self.env, self.project_name + ".uvproj", groups)
            print("Keil4 project is generating...")
        elif os.path.isfile("template.Uv2") and target_name not in ["mdk4", "mdk5"]:
            MDK2Project(self.env, self.project_name + ".Uv2", groups)
            print("Keil2 project is generating...")
        else:
            raise RuntimeError("Keil template missing.")
